...
/**
 * A fixed identity, so no test depends on the host's git config: a CI runner
 * has none, and `git merge` then stops before it merges anything.
 */
...
/** Like `git`, but returns the result instead of throwing, for commands expected to fail. */
export function tryGit(cwd: string, args: readonly string[]): SpawnSyncReturns<string> {
