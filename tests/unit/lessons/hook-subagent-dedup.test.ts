...
import { alwaysLesson, graphOf, useHookProject } from './hook-test-helpers.js';
...
function edit(sessionId: string, agentId?: string): Record<string, unknown> {
  return {
...
describe('hook dedup is scoped per agent context', () => {
  ...
  it('keeps UserPromptSubmit always-on lessons per agent context too', async () => {
    ...
    const prompt = (agentId?: string): Record<string, unknown> => ({
      session_id: s,
      ...(agentId !== undefined ? { agent_id: agentId } : {}),
      hook_event_name: 'UserPromptSubmit',
      prompt: 'touch db migrations',
    });
    saveLessonsGraph(project.root(), {
      ...graphOf({}),
      lessons: { aw: alwaysLesson('Universal rule.') },
    });
    expect(await run(prompt())).toContain('Universal rule.');
    expect(await run(prompt())).toBe('');
    expect(await run(prompt('a1'))).toContain('Universal rule.');
  });
});
