...
- **Output directory safety.** `generate` and `convert` reject output directories that resolve outside the project (or home directory with `--global`). Stale-file cleanup enforces the same boundary.
