...
- **Safe adoption.** `import`, then `diff`, then `generate`, then `check`; nothing is overwritten blind. When several tools have a root rule, they are merged into one `_root.md` rather than the last one winning, and their permissions, ignore patterns and MCP servers add up instead of replacing each other. [Existing-project guide](https://samplexbro.github.io/agentsmesh/guides/existing-project/)
