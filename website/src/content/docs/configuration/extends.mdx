...
This means your local overrides always take precedence over shared org config. The root rule follows the same order, even when the rules have different names: there is only one, taken from your local `.agentsmesh/` if it has one, else from an installed pack, else from an extended source. Any other rule marked `root: true` is used as a normal rule, and `generate` prints a warning naming it. The one exception is hooks from installed packs, which are kept next to yours; see [community packs](../../guides/community-packs/#merge-behavior).
