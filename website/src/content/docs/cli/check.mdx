...
The MCP `check` tool mirrors this in its result payload, exposing `canonicalDrift`, `outputDrift`, `outputsModified`, `outputsRemoved`, `outputsStale`, `outputsChecked`, and `lockConflict` (true when the lock has git conflict markers; run `agentsmesh merge`).
