...
Before writing output, `convert` rejects destination directories that resolve outside the project (or home directory with `--global`), including escapes through symlinks. Symlinks to directories inside that boundary remain supported.
