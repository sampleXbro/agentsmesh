...
Output directories must resolve inside the project, or inside the home directory with `--global`. Generation checks output paths before writing files and rejects symlinked directories that escape that boundary. Stale-file cleanup also rejects paths outside the boundary. Symlinks to directories within the boundary remain supported.
