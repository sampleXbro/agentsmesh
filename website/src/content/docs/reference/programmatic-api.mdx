...
Like `agentsmesh import`, it adds to the canonical permissions, ignore patterns and MCP servers already on disk instead of replacing them. See [Importing more than one tool](../../cli/import/#importing-more-than-one-tool).
...
if (report.lockConflict) {
  console.error('The lock file has git conflict markers — run agentsmesh merge.');
  process.exit(1);
}
...
`LockSyncReport.staleTargets` lists enabled targets that a `generate --targets` run left out after the canonical sources changed. Their files were not regenerated, so `inSync` is `false` (and `outputDrift` is `true`) until they are generated again.

`LockSyncReport.lockConflict` is `true` when `.lock` still has git conflict markers. Such a lock cannot be read, so `hasLock` is `false` too. Check `lockConflict` first: the fix is `agentsmesh merge`, not `generate`.
