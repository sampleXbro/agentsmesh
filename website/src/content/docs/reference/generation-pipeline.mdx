...
Filtered generates (`--targets`/`--features`) merge their outputs per-path into the existing `outputs` map without pruning stale entries; a full `agentsmesh generate` replaces the map, which is when entries for disabled targets drop off. `agentsmesh merge` omits the `outputs` map entirely, so output verification is reported as skipped until you regenerate.
