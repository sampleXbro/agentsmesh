...
migrator turns every list item under a `## Rules` or `## Lessons` heading of a
topic file into a lesson in `lessons.json` (numbered or bullet, with the lines
that wrap onto it) and **deletes `index.yaml` and `topics/`**. A list item under
any other heading stops the migration with nothing changed. `journal.md` is kept,
because its notes are not rules:
...
is absent and `index.yaml` exists. The recall hook never runs it. New projects skip this entirely.
