...
| `CORRUPT_GRAPH`             | error   | `lessons.json` could not be parsed at all (bad JSON, but no conflict markers). Recall degrades to empty while this stands. Keep a copy first, then repair the JSON by hand or restore the last committed graph from git (that drops lessons not committed yet). Recall's warning prints the same recovery steps.                                                                                                                                                                                                                                                        |
