...
2. **Verify the canonical files**
   ...
   <Aside type="note" title="Several tools with root rules">
     ...
     When two tools have a rule, command or agent with the same name and different text,
     `init --yes` keeps both and saves the later one as `<name>-<tool>.md`; review them.
     With separate `agentsmesh import` runs, commands, skills, hooks and same-name rules
     are last-import-wins.
   </Aside>
